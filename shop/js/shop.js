(function(){
  ...
  // API helpers — cart persistent storage on the server (session bound)
  ...
  function updateCartCount(){
    // fast immediate update using localStorage
    try{
      const cart = getCart();
    ...
    // then try to sync with server and correct UI if needed
    serverGetCart().then(serverCart => {
      const el = document.getElementById('cart-count');
      if(el){
        const count = Object.values(serverCart).reduce((s,v)=>s+v,0);
        el.textContent = count;
      }
      // also keep local copy in sync with server
      try{ saveCart(serverCart); }catch(e){}
  ...
  function addToCart(id, qty){
    ...
    // try to persist on server (fire-and-forget; fallback kept local)
    serverModify('add', { product_id: id, quantity: qty }).then(serverCart=>{
      try{ saveCart(serverCart); }catch(e){}
      updateCartCount();
    }).catch(()=>{/* ignore network/server problems */});
  }
  ...
  function renderCart(){
    ...
    if(!el) return;
    // prefer server cart when available, otherwise fallback to local
    let cart = {};
    try{
      // attempt server first
      const serverCart = await (async()=>{
        try{ return await serverGetCart(); }catch(e){ return null; }
      })();
      if(serverCart){ cart = serverCart; }
      else { cart = getCart(); }
  ...
  function initCheckout(){
    ...
    form.addEventListener('submit', async (e)=>{
      e.preventDefault();
      // simulate order — clear both local and server cart
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    fetch(API_AUTH, { credentials: 'same-origin' }).then(r=>r.json()).then(b=>{
      if(b && b.success){ csrfToken = b.csrf || null; currentUser = b.user || null; }
    }).then(()=>{ updateCartCount(); updateAuthUI(); }).catch(()=>{ updateCartCount(); updateAuthUI(); });
